...
- `scroll-button`: <Since version="0.1.10" /> the button code used for the `on-button-down` scroll method. You can find it in `libinput debug-events`.
...
- `drag-lock`: <Since version="25.0.2" /> if set, lifting the finger off for a short time while dragging will not drop the dragged item. See the [libinput documentation](https://wayland.freedesktop.org/libinput/doc/latest/tapping.html#tap-and-drag).
...
- `scroll-factor`: <Since version="0.1.10" /> scales the scrolling speed by this value.

Settings specific to `tablet`s:

- `calibration-matrix`: <Since version="25.0.2" /> set to six floating point numbers to change the calibration matrix. See the [`LIBINPUT_CALIBRATION_MATRIX` documentation](https://wayland.freedesktop.org/libinput/doc/latest/device-configuration-via-udev.html) for examples.
...
<Since version="0.1.7" /> When a tablet is not mapped to any output, it will map to the union of all connected outputs, without aspect ratio correction.
...
<Since next-release /> You can customize this with the `mode` property.
...
<Since version="0.1.8" /> You can optionally set `max-scroll-amount`.
...
### `mod-key`, `mod-key-nested` <Since next-release />

Customize the `Mod` key for [key bindings](./key-bindings).
